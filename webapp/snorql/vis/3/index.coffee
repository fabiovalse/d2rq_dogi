width = d3.select('body').node().getBoundingClientRect().width*80/100
height = d3.select('body').node().getBoundingClientRect().height*70/100
...
redraw = (flag) ->
  d3.json 'tree.json', (error, root) ->
    ...
    nodes.sort (a,b) -> d3.ascending a.name, b.name
    ...
    sectors_one.select 'title'
      .text (d) -> "#{d.name}" + (if flag then "\n#{d.value}" else "\n#{d.size}") + (if d.desc? then "\n#{d.desc}" else "")
